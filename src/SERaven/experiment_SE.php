...
<html>
      <head>
            ...
            <script   src  = 'SERaven.js'></script>
            <script   src  = 'sampling-master/discrete.js'></script>
...
<script type="application/javascript">
  ...
  // Feedback regime betas // b0 b1 logAlpha
  const fb_regime = {PSE: [[-3,   5, 1.5],  // Domain 1 - Effort matters
                           [-3, 1.5, 1.5]]} // Domain 2 - Effort doesn't matter

  // instructions
  const nbInstr              = 7;

  // stimuli
  const nbStimuli            = [1,1,1];
  ...
  if (browserInfo.browser !== 'Chrome' && browserInfo.browser !== 'Firefox') {
    ...
    function sigmoid(x) { // sigmoid of x
    ...
    // Block 1 stimuli - real
    var block1Stimuli = [];
    var block1Stimuli_html = [];
    for (var t=1; t <= nbStimuli[0]; t++){
      block1Stimuli[t-1] = 'Stimuli/Ravens/Fake/Slide0'+6+'.BMP'; // pre-load all instructions
      block1Stimuli_html[t-1] = '<img src="'+block1Stimuli[t-1]+'"  id="image-instructions" style="height:'+screen.height/1.25+'px"></img>';
    }

    // Block 2 stimuli - fake
    var block2Stimuli = [];
    var block2Stimuli_html = [];
    for (var t=1; t <= nbStimuli[1]; t++){
      block2Stimuli[t-1] = 'Stimuli/Ravens/Fake/Slide0'+7+'.BMP'; // pre-load all instructions
      block2Stimuli_html[t-1] = '<img src="'+block2Stimuli[t-1]+'"  id="image-instructions" style="height:'+screen.height/1.25+'px"></img>';
    }

    // Block 3 stimuli - real
    var block3Stimuli = [];
    var block3Stimuli_html = [];
    for (var t=1; t <= nbStimuli[2]; t++){
      block3Stimuli[t-1] = 'Stimuli/Ravens/Fake/Slide0'+8+'.BMP'; // pre-load all instructions
      block3Stimuli_html[t-1] = '<img src="'+block3Stimuli[t-1]+'"  id="image-instructions" style="height:'+screen.height/1.25+'px"></img>';
    }

    function updateLoadedCount(nLoaded){
      var percentcomplete = Math.min(Math.ceil(nLoaded / (instrImg.length + nbStimuli.reduce((a, b) => a + b, 0) + 1)  * 100), 100);
...
// ------------------------------ BEGIN EXPERIMENT --------------------------- //
    ...
    if (cfg.debug == false) {
      jsPsych.pluginAPI.preloadImages([instrImg, block1Stimuli, block2Stimuli, block3Stimuli],
    ...
    function startExperiment(){
      ...
      var task = SERaven(exp.nbBlocks, exp.nbTrials, exp);
