function SE2TrainingTimeline(){
  ...
  var targetLocation  = Array(train_TS);
  var correctLocation = Array(train_TS);
  var testTrials      = [];
  ...
  for (var testInd = 0; testInd < train_TS; testInd++) {
    ...
    target_i_train[testInd]      = train_grid_indexes[0][testInd][pair_1st].map(function(v){return (v - 1)})
    target_corr_i_train[testInd] = train_grid_indexes[0][testInd][pair_2nd].map(function(v){return (v - 1)})

    test_trials_train.push({
      target_location:  target_i_train[testInd],
      correct_location: train_grid_indexes[0][testInd][pair_2nd].map(function(v){return (v - 1)}),
