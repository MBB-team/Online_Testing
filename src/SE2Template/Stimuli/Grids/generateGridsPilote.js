/* Generate the grids for the pilote

//AUTHOR: Nour Mohsen 
//DATE : March 29th, 2024 
*
* Taken from Will Hopper - 30/04/2020
...
*/
//var gridStimuliTrain = generateGridsPilote(numbersImg, gridIndexesTrain, squareSize, [0], [0], [1]);

function generateGridsPilote(numbersImg, gridIndexesPt, TSPt, gridsPt, lettersPt){

   console.log("The Grid Indexes here are:", gridIndexesPt)
    ...
    for (var grid_i = 0; grid_i <= nbTrials-1; grid_i++){ // for each grid
       ...
       gridCounter++
       console.log("Creating grid #:", gridCounter)

        squareSize = exp.squareSize[gridsPt[grid_i]];
        n_TS = exp.TS[TSPt[grid_i]];        // get the current TS
        grid = exp.grids[gridsPt[grid_i]]; //get current gridsize
        console.log("the current grid size is:", grid) //NOTES FOR MYSELF
        var letterStatus = lettersPt[grid_i] == 1 ? 'yes':'no';
        console.log("The letter index is", lettersPt[grid_i])
        console.log("This grid will have letters?", letterStatus)
  
        var grid_num = new Array(grid.length).fill(0).map(() => new Array(grid[0].length).fill(999));
    
    
        if (lettersPt[grid_i] === 1) {
            // for half the stimuli the empty squares of the grid will be filled with randomly pulled numbers
      ...
      for(var i=0; i<grid.length; i++){
        ...
        for(var j=0; j<grid[i].length; j++){
          ...
          // grid filling - conditional to letter filling 
          if (lettersPt[grid_i] === 1) {
            if (grid_num[i][j] != 999){
            ...
            if (grid_num[i][j] != 999){
              ...
              filler_counter++;
              stimulus += '<img src="'+emptyImg+'" style="height:'+squareSize+'px; width:auto"></img>';
            }
