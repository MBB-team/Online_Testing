...
<html>
      <head>
            ...
            <script   src  = 'Stimuli/Grids/SE2Template - Task Conditions.js'></script>  <!-- NEED TO CHANGE -->
...
<script type="application/javascript">
  ...
  const exp = {name:           "SE2Template",
               ...
               squareSize:    screen.height/8}; // can maybe even / 7 ? 
  ...
  // instructions
  const nbInstr              = 23; //Change if there are changes 
  ...
  if (browserInfo.browser !== 'Chrome' && browserInfo.browser !== 'Firefox') {
    ...
    var gridStimuliTrain = generateGridsTrain(numbersImg, gridIndexesTrain, exp.squareSize, matchingPairs, 4, exp.grid);
    
    /*
    console.log("Start normals grids")
    var gridStimuli      = generateGridsMain(numbersImg, gridIndexesPt, exp.squareSize, matchingPairs, TSPt, exp.grid);
   */
